...
/// Batch-translate all story text in one HTTP round-trip and return a
/// populated dict. Replaces the old per-string sequential approach which
/// spawned a thread and called translate_one() N times.
fn generate_auto_tl_dict(this: *mut Il2CppObject) -> Result<StoryTimelineDataDict, Error> {
    ...
    // Step 2: Send it to the tl server in one batch
