use crate::{
    core::{hachimi::UITextConfig, game::Region, Hachimi},
    il2cpp::{
...
use super::{GachaExecutableUnit, PartsHorizontalTextSet};
...
static mut DAILY_TEXT_SET_FIELD: *mut FieldInfo = null_mut();
static mut DRAW_COUNT_TEXT_SET_FIELD: *mut FieldInfo = null_mut();
static mut DRAW_COUNT_TEXT_FIELD: *mut FieldInfo = null_mut();
static mut EXECUTABLE_FIELD: *mut FieldInfo = null_mut();
...
extern "C" fn SetButtonText(this: *mut Il2CppObject) {
    ...
    let executable = get_executable(this);
...
fn apply_gacha_button_config(this: *mut Il2CppObject, executable: *mut Il2CppObject) {
    ...
    let text_set = get_drawCountTextSet(this);
    let daily_set = get_dailyTextSet(this);
    let draw_count_text = get_drawCountText(this);
...
pub fn init(umamusume: *const Il2CppImage) {
    ...
    if Hachimi::instance().game.region == Region::Japan {
        let Initialize_addr = get_method_addr(PartsGachaButton, c"Initialize", 8);
        new_hook!(Initialize_addr, InitializeJp);
    }
    else {
        let Initialize_addr = get_method_addr(PartsGachaButton, c"Initialize", 6);
        new_hook!(Initialize_addr, InitializeOther);
    }

    let SetButtonText_addr = get_method_addr(PartsGachaButton, c"SetButtonText", 0);
