...
extern "C" fn CompressRequest(data: *mut Il2CppArray) -> *mut Il2CppArray {
    ...
    match result {
        Ok(res) => res,
        Err(_) => {
            log::error!("Panic in CompressRequest caught!");
            get_orig_fn!(CompressRequest, CompressRequestFn)(data)
        }
    }
}
...
extern "C" fn DecompressResponse(compressed: *mut Il2CppArray) -> *mut Il2CppArray {
    ...
    match result {
        Ok(res) => res,
        Err(_) => {
            log::error!("Panic in DecompressResponse caught!");
            get_orig_fn!(DecompressResponse, DecompressResponseFn)(compressed)
        }
    }
}
